...
def populate():
    jabba_user = CraggUser.objects.get_or_create(username='jabba', first_name='Jack',
                          last_name='Johnson', user_email='jabba@fake.net',
                          user_password='jabbajohn')[0]
    jabba_user.save()

    luke_user = CraggUser.objects.get_or_create(username='luke', first_name='Luke',
                         last_name='Skywalker', user_email='theforce@astro.com',
                         user_password='lukeboyee')[0]
    luke_user.save()

    chewy_user = CraggUser.objects.get_or_create(username='chewy', first_name='Chewbacca',
                          last_name='Bigfoot', user_email='chewy@falcon.org',
                          user_password='chewyloveshan')[0]
    chewy_user.save()

    boba_user = CraggUser.objects.get_or_create(username='boba', first_name='Boba',
                         last_name='Fett', user_email='bobafett@lasergun.net',
                         user_password='murderbydeath')[0]
    boba_user.save()

    for u in CraggUser.objects.all():
